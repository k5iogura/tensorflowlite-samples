...
with open('mnist_frozen.pb','rb') as f:
    graph_def.ParseFromString(f.read())
    tf.import_graph_def(graph_def, name='')
nodes = [i.name for i in graph_def.node]
print(nodes)

y_ = tf.placeholder(tf.float32, [None, 10])
x  = tf.get_default_graph().get_tensor_by_name('inputX:0')
y  = tf.get_default_graph().get_tensor_by_name('outputX:0')
